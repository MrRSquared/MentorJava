...
//PiCar Specific Imports
import edu.wpi.first.wpilibj.DigitalOutput; //for the motor controller
import frc.robot.AngleConversion; //for the steering
import edu.wpi.first.wpilibj.Servo;//for the steering

/**
 * This is a modified version of the WPILib DifferentialDrive class for use with the PiCar servo controlled wishbone steering and motor controller.
 * 
 * 
 ...
 */
public class CarDrive extends RobotDriveBase {
  ...
  //Some piCar Specific instances
  DigitalOutput leftMotorReverse;
  DigitalOutput rightMotorReverse;

  final static int leftReverseChannel = 8;  //TODO: create imports for these
  final static int rightReverseChannel = 9;

  private final AngleConversion m_arcadeTurnConverter = new AngleConversion();
  private final AngleConversion m_tankTurnConverter = new AngleConversion();
  private final Servo turnServo = new Servo(3); //TODO: Create imports for these
   ...
   * 
   */
  //-----> I could add to the constructor, by , [int leftReverseChannel, int rightReverseChannel, int turnServoChannel] but I want to Keep it simple in the beginning.  <---------------
  public CarDrive(SpeedController leftMotor, SpeedController rightMotor) {
    ...
    leftMotorReverse = new DigitalOutput(leftReverseChannel);  // Bring Input Over
    rightMotorReverse = new DigitalOutput(rightReverseChannel);//
    m_arcadeTurnConverter.setOldRange(-1,1);    //This translates the input from the stick to the 
    m_arcadeTurnConverter.setNewRange(27, 153);//
    m_tankTurnConverter.setOldRange(-2,2);    //This translates the input from the stick to the 
    m_tankTurnConverter.setNewRange(27, 153);//
  ...
  public void arcadeDrive(double xSpeed, double zRotation, boolean squareInputs) {
    ...
    //Use zRotation to set the servoAngle
    m_arcadeTurnConverter.setInputNumber(zRotation);
    m_arcadeTurnConverter.convert();
    int m_arcadeTurnOutput = (int) m_arcadeTurnConverter.getOutputNumber();
    turnServo.setAngle(m_arcadeTurnOutput);
    
    SmartDashboard.putNumber("servo Angle",  m_arcadeTurnOutput);
   // double maxInput = Math.copySign(Math.max(Math.abs(xSpeed), Math.abs(zRotation)), xSpeed);

    if (xSpeed >= 0.0) {
      leftMotorOutput = xSpeed;
      rightMotorOutput = xSpeed;

      // First quadrant, else second quadrant  
      //This is the old algorithm.  
      //CUrrently, we are only concerned with forward or backward motion.
      /** if (zRotation >= 0.0) {
      ...
      leftMotorReverse.set(true);  // Reverse the motors 
      rightMotorReverse.set(true); // Reverse the motors
      leftMotorOutput = xSpeed;
      rightMotorOutput = xSpeed;
  ...
  public void tankDrive(double leftSpeed, double rightSpeed, boolean squareInputs) {
    ...
    double tankConvertAngle = leftSpeed - rightSpeed;

    m_tankTurnConverter.setInputNumber(tankConvertAngle);
    m_tankTurnConverter.convert();
    int m_tankTurnOutput = (int) m_tankTurnConverter.getOutputNumber();
    turnServo.setAngle(m_tankTurnOutput);
    SmartDashboard.putNumber("tank angle", m_tankTurnOutput);
    SmartDashboard.putNumber("right Motor Output", rightSpeed);
    SmartDashboard.putNumber("left Motor Output", leftSpeed);
